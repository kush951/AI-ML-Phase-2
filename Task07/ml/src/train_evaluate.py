...
def run_pipeline():
    ...
    top3 = [(name, info["models"]) for name, info in sorted_models[:3]]
    ...
    print("\n[5/6] Selecting best models and evaluating on held-out test set …")
    ...
    # ── 8. Save Artefacts ─────────────────────────────────────────────
    print("\n[6/6] Saving models & metadata …")
    joblib.dump(best_model, MODELS_DIR / "best_model.pkl")

    # Save all models metrics for report
    metrics_export = {}
    for name, info in results.items():
        metrics_export[name] = {
            k: round(v, 6) for k, v in info.items() if k != "models"
        }
    ...
    print(f"     Saved models → {MODELS_DIR / 'best_model.pkl'}")
